<?php 
    $title = 'Professor $variable here'; // Title of Page
    include 'head.php'; 
    include 'nav.php';
    ?>
<!-- Styles -->
    ...
    <?php include 'left-menu.php' ?>
        <div class="col-8">
            ...
            <?php
                ...
                //SELECT * FROM `reviews` where `fname` LIKE '" . $id . "'  OR lname LIKE '" . $id . "' ORDER BY Timestamp DESC
                $query = "SELECT * FROM `reviews` where `fname` LIKE '" . $id . "'  OR lname LIKE '" . $id . "' ORDER BY Timestamp DESC";
                $result = mysqli_query($conn,$query);
                
                while($row = mysqli_fetch_array($result)) {
                echo "<h1 class=\"display-4\">".$row["fname"]." ".$row["lname"]."</h1>";
                echo $row['fname'];
                echo $row['lname'];
                echo $row['review'];
                echo $row['timestamp'];
            ...
            ?>
                <p class="lead">
                Department(s): <a href="#">English</a> 
                <br />
                Course(s): <a href="#">ENG 2850</a>
                </p>
            </div>
            <div class="card">
                <div class="card-body">
